...
public class Kinematics : MonoBehaviour {
	...
	public void StartKinematics()
	{
		...
		{
			var t = float.Parse(time.text);
			var x = float.Parse(inputX.text);
			var y = float.Parse(inputY.text);
			var z = float.Parse(inputZ.text);

			target = new Vector3(x, y, z);
			StartCoroutine(RotateBaseToTarget(t));
			StartCoroutine(ConfigureArm(t));
		}
	}
